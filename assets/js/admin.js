...
// Toggle Dropdown Sidebar Menu (Bisa via Klik atau Hover)
...
function navigateAdmin(pageKey) {
  ...
  // Buka dan tandai parent dropdown jika sub-item sedang aktif
  document.querySelectorAll('.nav-dropdown').forEach(dd => {
    const hasActive = !!dd.querySelector(`.admin-nav-item[data-page="${pageKey}"]`);
    dd.classList.toggle('has-active-child', hasActive);
    if (hasActive) {
...
function updateAdminStats() {
  ...
  // Group Badges pada Dropdown Sidebar
  const bgRenja = document.getElementById('badge-group-renja'); if (bgRenja) bgRenja.textContent = renjaCount;
  const bgPk = document.getElementById('badge-group-pk'); if (bgPk) bgPk.textContent = pkCount;
  const bgDpa = document.getElementById('badge-group-dpa'); if (bgDpa) bgDpa.textContent = dpaCount;
  const bgSurat = document.getElementById('badge-group-surat'); if (bgSurat) bgSurat.textContent = suratCount;
